...
import { LiveKitRoom } from "@/components/stream/LiveKitRoom";
...
export default function WatchPage() {
  ...
  const bootstrapPollingEnabled = !market && !isInitialLoad;
  ...
  useBootstrapMarketPolling(bootstrapPollingEnabled, refresh);
  ...
  // Broadcast overlay updates to the stream viewer overlay
          ...
          {selectedAuthority && (
            <div className="panel p-0 overflow-hidden lg:shadow-2xl">
              <div className="relative w-full bg-black aspect-video lg:min-h-[460px]">
                <ViewerStream authority={selectedAuthority.toBase58()} onMeta={setStreamMeta} hideOverlay={pollHidden} />
              </div>
          ...
          <section className={`panel p-5 space-y-5 relative ${lockAny ? 'opacity-60' : ''}`}>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-2 ml-auto">
                <button
                  type="button"
                  onClick={() => setPollHidden(prev => !prev)}
                  className="inline-flex items-center gap-1 rounded-md border border-white/15 bg-white/5 px-2.5 py-1 text-[11px] font-medium text-white/70 transition hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/30"
                  aria-pressed={pollHidden}
                >
                  {pollHidden ? 'Show Poll' : 'Hide Poll'}
                </button>
                {!pollHidden && (market || realtimeBadge) && (
                  <div className="flex items-center gap-2">
                    {market && (
                      <span
                        ...
                        aria-label={market.resolved ? 'Poll resolved' : market.frozen ? 'Poll frozen' : 'Poll live'}
                      >
            ...
            {!pollHidden && (
              <h2 className="mt-1 text-lg font-semibold leading-tight text-white break-words">{market?.title || 'Untitled Market'}</h2>
            )}
            {pollHidden ? null : isInitialLoad ? (
              ...
              <div className="space-y-4">
                ...
                {ticket && !market?.frozen && (
                  <div className="rounded-md border border-white/10 bg-black/25 p-4 text-xs text-white/75">
                    <p className="text-[11px] uppercase tracking-[0.25em] text-amber-300 font-semibold">Your position</p>
                ...
                <div className={`space-y-3 ${!userIsAuthority && !market.resolved ? 'border-t border-white/10 pt-3' : ''}`}>
                  {!market.resolved ? (
                    ...
                    <div className="rounded-lg border border-white/10 bg-black/30 p-4 space-y-3" style={{ minWidth: 0 }}>
                      ...
                      <PollSummaryBar
                        ...
                        variant="viewer"
                        highlightSide={market.winningSide === 0 ? 'yes' : market.winningSide === 1 ? 'no' : undefined}
                      />
                      <div className="grid grid-cols-2 gap-2 text-[11px] text-white/70">
                        <div className={`rounded-md border px-3 py-2 ${market.winningSide === 0 ? 'border-emerald-400 bg-emerald-500/15 text-emerald-100' : 'border-white/5 bg-white/5'}`}>
                          <p className="uppercase tracking-[0.25em] text-white/40">{market.labelYes || 'YES'}</p>
                          <p className="text-sm font-semibold text-white mt-1">{yesShare > 0 ? `${yesShare.toFixed(yesShare >= 10 ? 1 : 2)}%` : '—'}</p>
                        </div>
                        <div className={`rounded-md border px-3 py-2 ${market.winningSide === 1 ? 'border-[#f87171] bg-[#b91c1c]/20 text-[#fecaca]' : 'border-white/5 bg-white/5'}`}>
                          <p className="uppercase tracking-[0.25em] text-white/40">{market.labelNo || 'NO'}</p>
                          <p className="text-sm font-semibold text-white mt-1">{noShare > 0 ? `${noShare.toFixed(noShare >= 10 ? 1 : 2)}%` : '—'}</p>
                        </div>
                      </div>
...
function ViewerStream({ authority, onMeta, hideOverlay }: { authority: string; onMeta?: Dispatch<SetStateAction<{ active: boolean; viewerCount: number; title: string | null } | null>>; hideOverlay?: boolean }) {
  const identityRef = useRef<string>("");
  if (!identityRef.current) {
    const stub = authority.slice(0, 8).toLowerCase();
    const randomSegment = Math.random().toString(36).slice(2, 8);
    identityRef.current = `viewer-${stub}-${randomSegment}`;
  }
  ...
  const overlayNode = overlay && !hideOverlay ? (
